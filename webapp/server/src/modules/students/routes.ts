...
const updateSchema = z
  .object({
    ...
    // Required alongside newPassword — a valid session alone shouldn't be enough
    // to change the password (e.g. a hijacked or forgotten-logout session).
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (body.newPassword) {
      ...
      const existing = await prisma.user.findUniqueOrThrow({ where: { id: req.user!.userId } });
